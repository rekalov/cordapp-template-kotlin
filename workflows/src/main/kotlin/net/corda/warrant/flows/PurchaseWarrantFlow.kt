...
@InitiatingFlow
@StartableByRPC
class PurchaseWarrantFlow(
        private val basket: List<AmountTransfer<TokenPointer<Warrant>, Party>>)
    ...
    override fun call(): SignedTransaction {
        logger.info("zZA1-PurchaseWarrantFlow start")
    ...
    progressTracker.currentStep = QUERYING_WARRANTS
        ...
        // TODO collect in one DB query
        // XXX what if we have multiple versions of the same Warrant?
        val warrantStates = basket.map { it.token }.toSet()
                .map { it to getStateReference(Warrant::class.java, it.pointer.pointer).state.data }.toMap()
    ...
    progressTracker.currentStep = sessionSteps[sessionIndex].requestingTransfers
            ...
            val warrantTransferRequests = basket.filter { it.source == session.counterparty }.map {
                Pair(it.destination, Amount(it.quantityDelta, it.token as TokenType))
            }
            val cashTransferRequests = basket.filter { it.destination == session.counterparty }.map {
                Pair(it.source, warrantStates[it.token]!!.price
                        // TODO here we assume warrant quantityDelta has 0 decimal points
                        .let { price -> price.copy(quantity = price.quantity * it.quantityDelta) } )
            }
            val transferRequest = TokenTransferRequest(lockId, warrantTransferRequests + cashTransferRequests)
    ...
    progressTracker.currentStep = RECORDING
        ...
        logger.info("zZA1-PurchaseWarrantFlow-UpdateDistributionListFlow start disabled")
     ...
     // logger.info("zZA2-PurchaseWarrantFlow-UpdateDistributionListFlow finish")
        ...
        logger.info("zZA1-PurchaseWarrantFlow finish")
    ...
    @InitiatedBy(PurchaseWarrantFlow::class)
    class PurchaseWarrantFlowResponse (private val flowSession: FlowSession): FlowLogic<Unit>() {
        @Suspendable
        override fun call() {
            logger.info("zZA1-PurchaseWarrantFlowResponse start")
            ...
            if (transferRequests.isNotEmpty()) {
                ...
                logger.info("zZA2-PurchaseWarrantFlowResponse-SendStateAndRefFlow start")
                subFlow(SendStateAndRefFlow(flowSession, allInputs))
                logger.info("zZA2-PurchaseWarrantFlowResponse-SendStateAndRefFlow finish")

                logger.info("zZA2-PurchaseWarrantFlowResponse-send(allOutputs) start")
                flowSession.send(allOutputs)
                logger.info("zZA2-PurchaseWarrantFlowResponse-send(allOutputs) finish")
            }

            logger.info("zZA2-PurchaseWarrantFlowResponse-subFlow(ConfidentialTokensFlowHandler) start")
            subFlow(ConfidentialTokensFlowHandler(flowSession))
            logger.info("zZA2-PurchaseWarrantFlowResponse-subFlow(ConfidentialTokensFlowHandler) finish")

            val signTransactionFlow = object : SignTransactionFlow(flowSession) {
                override fun checkTransaction(stx: SignedTransaction) {
                }
            }
            logger.info("zZA2-PurchaseWarrantFlowResponse-subFlow(signTransactionFlow) start")
            subFlow(signTransactionFlow)
            logger.info("zZA2-PurchaseWarrantFlowResponse-subFlow(signTransactionFlow) finish")

            // TODO raise token sdk issue - inability to pass transactionId
            logger.info("zZA2-PurchaseWarrantFlowResponse-ObserverAwareFinalityFlowHandler start")
            subFlow((ObserverAwareFinalityFlowHandler(flowSession)))
            logger.info("zZA2-PurchaseWarrantFlowResponse-ObserverAwareFinalityFlowHandler finish")

            logger.info("zZA2-PurchaseWarrantFlowResponse-UpdateDistributionListFlowHandler start disabled")
         ...
         // logger.info("zZA2-PurchaseWarrantFlowResponse-UpdateDistributionListFlowHandler finish")
            logger.info("zZA1-PurchaseWarrantFlowResponse finish")
        }
    }
